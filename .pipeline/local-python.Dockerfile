...
FROM docker-registry.wikimedia.org/python3-build-buster:0.0.2-s1 AS dockerize
USER 0
ENV HOME="/root"
...
FROM docker-registry.wikimedia.org/python3-build-buster:0.0.2-s1 AS local-python
USER 0
ENV HOME="/root"
ENV DEBIAN_FRONTEND="noninteractive"
...
LABEL blubber.variant="local-python" blubber.version="0.8.0+d91393a"
