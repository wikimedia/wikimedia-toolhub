...
FROM docker-registry.wikimedia.org/python3-build-buster AS prep
USER "root"
ENV HOME="/root"
...
FROM docker-registry.wikimedia.org/python3-build-buster AS demo
USER "root"
ENV HOME="/root"
ENV DEBIAN_FRONTEND="noninteractive"
RUN apt-get update && apt-get install -y "build-essential" "default-libmysqlclient-dev" "gettext" "python3-venv" && rm -rf /var/lib/apt/lists/*
...
USER "runuser"
ENV HOME="/home/runuser"
ENTRYPOINT ["/bin/bash", "-c", "poetry run python3 manage.py migrate && poetry run python3 manage.py loaddata toolhub/fixtures/demo.yaml && poetry run python3 manage.py crawl && poetry run python3 manage.py runserver --noreload --nostatic 0.0.0.0:8000"]

LABEL blubber.variant="demo" blubber.version="0.8.0+f063588"
